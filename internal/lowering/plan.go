	...
	for _, bb := range f.Blocks {
		for _, i := range bb.Instrs {
			...
			if call, ok := i.(ssa.CallInstruction); ok {
				...
				if summary.Kind == effects.ScalarFormat {
					if site, ok := call.(*ssa.Call); ok {
						if proof := b.effects.ProveScalarFormat(site); proof != nil {
				...
				if !summary.IsPure() || summary.Kind == effects.FiniteData || summary.Kind == effects.ConstantData || summary.Kind == effects.ScalarFormat || call.Common().IsInvoke() && summary.Callee != nil || b.p.CallableField(call) != nil {
