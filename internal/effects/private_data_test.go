...
func TestPrivateDataConstructorSummaries(t *testing.T) {
	for _, tc := range []struct {
		...
		{"array-slice-alias", "", `func ctor()*[2]int{p:=new([2]int);s:=p[:];s[1]=2;return p}`, effects.Inspect},
		...
		{"readonly-escape", "", `func ctor()*record{p:=&record{text:"message"};observe(p);return p}`, effects.Inspect},
