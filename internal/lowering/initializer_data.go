...
func (b *builder) consumeInitializerData(f *ssa.Function) bool {
	proof := b.effects.ProveInitializerDataEffects(f)
	if proof == nil {
		return false
	}
	for _, operation := range proof.ModeledOperations {
		...
		b.diag("info", "modeled-data-operation", operation, f.Pos())
	}
